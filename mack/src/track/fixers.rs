...
use crate::track::feat::{TrackFeat, extract_feat};
...
static MULTI_WS_RE: std::sync::LazyLock<Regex> =
...
pub fn run_fixers(track: &mut Track, dry_run: bool) -> Result<bool> {
    ...
    let new_title = fix_title(tags.title().as_deref(), tags.artist().as_deref());
    let new_artist = fix_artist(tags.artist().as_deref());
    let new_album = fix_album(tags.album().as_deref());
    ...
    if !dry_run && changed {
        tags.save_to_path(&track.path, WriteOptions::default())?;
    }
