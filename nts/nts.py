...
class NTSTracklistFetcher:
    ...
    def extract_episode_path(self, url: str) -> str | None:
        ...
        try:
            ...
            if (
                len(path_parts) >= 4
                and path_parts[0] == "shows"
                and path_parts[2] == "episodes"
            ):
    ...
    def extract_episode_name(self, url: str) -> str | None:
    ...
    def get_episode_data(self, episode_path: str) -> dict[str, Any]:
    ...
    def save_tracklist(self, episode_data: dict[str, Any], filename: str) -> None:
        """Save tracks to a text file."""
        if "--debug" in sys.argv:
            print(
                f"DEBUG: Response data: {json.dumps(episode_data, indent=2)}",
                file=sys.stderr,
            )

        # Extract tracks from the embeds.tracklist.results path
        embeds = cast(dict[str, Any], episode_data.get("embeds", {}))
        tracklist_container = cast(dict[str, Any], embeds.get("tracklist", {}))
        ...
        with open(filename, "w", encoding="utf-8") as f:
            for track in tracklist:
                artist = cast(str, track.get("artist", "Unknown Artist"))
                title = cast(str, track.get("title", "Unknown Title"))
                line = f"{artist} - {title}"
                print(line)
                f.write(line + "\n")
...
def main():
    ...
    _ = parser.add_argument("url", help="URL of the NTS Radio show")
    _ = parser.add_argument("--debug", action="store_true", help="Enable debug output")
